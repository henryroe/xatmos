...
#TODO:  selected line's wavenumber should be available in a click and copy form
...
hitran_files = glob.glob('hitran_abridged*txt*')
molecules = {}
for curfile in hitran_files:
    curname = re.match('^.*hitran_abridged_(.*)\.txt.*$', curfile).groups()[0]
    molecules[curname] = {'hitran_filename':curfile, 'hitran':None,
                          'plot_lines':None, 'plot_text':None, 'color':colors.next()}
    ...
    trait_view = View(VGroup(Item('figure', editor=mplFigureEditor, show_label=False),
                             HGroup('10',
                                    VGroup('40',
                                           ...
                                           Item(name='bandwidth',
                                                editor=TextEditor(auto_set=False, enter_set=True)),
# TODO: get next lines working to allow user to copy, but not edit, the selected line wavenumber
#                                            HGroup(Item(name='selected_line_wavenumber',
#                                                        editor=TextEditor(auto_set=True, enter_set=True),
#                                                        visible_when='selected_line_wavenumber>0'),
#                                                   show_border=True),
                                           show_border=True),
                                    HGroup(
    ...
    def __init__(self):
        ...
        self.texes_orders = pandas.io.parsers.read_csv('orders.txt', sep='\t', header=None, skiprows=3)
        self.atmos = pandas.io.parsers.read_csv(gzip.open('atmos.txt.gz', 'r'), sep='\t', skiprows=7, index_col='# wn')
    ...
    def on_click(self, event):
        if self.selected_line is not None:
            self.axes.lines.pop(self.axes.lines.index(self.selected_line[0]))
        ...
        self.selected_line = self.axes.plot([closest['wn'], closest['wn']], [0, 1], '-.', color='black')
    ...
    def replot_molecular_overplots(self):
        for i, cur_molecule in enumerate(self.selected_molecules):
            ...
            self.molecule_lookup_points[cur_molecule] = {'wn':wn, 'y':intensity + i * 0.1}
            wn = wn.repeat(3)
            intensity = np.column_stack((np.zeros(len(intensity)),
                                         intensity,
                                         np.zeros(len(intensity)))).flatten() + i * 0.1
            ...
            if self.molecules[cur_molecule]['plot_lines'] is not None:
                self.axes.lines.pop(self.axes.lines.index(self.molecules[cur_molecule]['plot_lines'][0]))
                self.molecules[cur_molecule]['plot_lines'] = None
            if self.molecules[cur_molecule]['plot_text'] is not None:
                self.axes.texts.remove(self.molecules[cur_molecule]['plot_text'])
                self.molecules[cur_molecule]['plot_text'] = None
            self.molecules[cur_molecule]['plot_lines'] = newplot
